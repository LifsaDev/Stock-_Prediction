...
stocks = ('GOOG', 'AAPL', 'MSFT', 'GME','AMZN','ABC','ADM','ADP','AEE')
...
#____________________________________MODEL LSTM_____________________________________________________________
 ...
 #_____________________________BUILD METHODE TO MAKE NEXTS DAYS PREDICT ____________________________________________
    ...
    if model_name =='RNN':
        ...
        plt.axhline(y=final_graph[len(final_graph)-1], color = 'red', linestyle = ':', label = 'your predict value : {0}'.format(round(float(*final_graph[len(final_graph)-1]),2)))
        plt.legend()
        st.write("show your prediction with RNN Model")
        st.pyplot(fig)
        ...
        plt.xlabel("Time")
        plt.title("{0}, next predicted  ".format(selected_stock))
        plt.axhline(y=data_final[len(data_final)-1], color = 'red', linestyle = ':', label = 'your predict value : {0}'.format(round(float(*data_final[len(data_final)-1]),2)))
        plt.legend()
        st.write("show your prediction with SVR Model")
        st.pyplot(fig)
        ...
        plt.axhline(y=data_final[len(data_final)-1], color = 'red', linestyle = ':', label = 'your predict value : {0}'.format(round(float(*data_final[len(data_final)-1]),2)))
        plt.legend()
        st.write("show your prediction with MLP Model")
        st.pyplot(fig)
        ...
        plt.axhline(y=data_final[len(data_final)-1], color = 'red', linestyle = ':', label = 'your predict value : {0}'.format(round(float(*data_final[len(data_final)-1]),2)))
        plt.legend()
        st.write("show your prediction with DT Model")
        st.pyplot(fig)
    elif model_name=="KNN":
